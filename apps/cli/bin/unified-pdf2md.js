...
Batch commands:
  ...
  unified-pdf2md review-folder --inputFolderPath in --outputFolderPath out [--recursive]
  unified-pdf2md export-folder --inputFolderPath in --outputFolderPath out [--recursive] [--translated] [--body-only] [--safe-only]
`
}
...
function assertFolderExists (folderPath, label) {
  if (!fs.existsSync(folderPath)) throw new Error(`${label} does not exist: ${folderPath}`)
  if (!fs.statSync(folderPath).isDirectory()) throw new Error(`${label} is not a folder: ${folderPath}`)
}
...
async function commandExport (args) {
  ...
  const markdown = exportMarkdown(document, exportOptionsFromArgs(args))
...
async function commandConvertFolder (args) {
  ...
  assertFolderExists(inputFolder, 'Input folder')
  const recursive = Boolean(args.recursive)
  const files = listPdfFiles(inputFolder, recursive)
  ensureAtLeastOneFile(files, inputFolder, 'PDF')
  ...
  console.log(`Converted ${files.length} PDF file(s).`)
}

async function commandReviewFolder (args) {
  const inputFolder = args.inputFolderPath || args.input
  const outputFolder = args.outputFolderPath || args.output
  if (!inputFolder) throw new Error('review-folder requires --inputFolderPath')
  if (!outputFolder) throw new Error('review-folder requires --outputFolderPath')
  assertFolderExists(inputFolder, 'Input folder')
  const recursive = Boolean(args.recursive)
  const files = listPdfFiles(inputFolder, recursive)
  ensureAtLeastOneFile(files, inputFolder, 'PDF')
  for (const filePath of files) {
    const rel = path.relative(inputFolder, filePath).replace(/\.pdf$/i, '.review.json')
    const out = path.join(outputFolder, rel)
    const document = await convertPdfToDocument(filePath)
    writeJson(out, document)
    console.log(`Wrote ${out}`)
  }
  console.log(`Created ${files.length} review JSON file(s).`)
}

async function commandExportFolder (args) {
  const inputFolder = args.inputFolderPath || args.input
  const outputFolder = args.outputFolderPath || args.output
  if (!inputFolder) throw new Error('export-folder requires --inputFolderPath')
  if (!outputFolder) throw new Error('export-folder requires --outputFolderPath')
  assertFolderExists(inputFolder, 'Input folder')
  const recursive = Boolean(args.recursive)
  const files = listReviewJsonFiles(inputFolder, recursive)
  ensureAtLeastOneFile(files, inputFolder, 'review JSON')
  for (const filePath of files) {
    const rel = reviewJsonRelativeMarkdownPath(inputFolder, filePath)
    const out = path.join(outputFolder, rel)
    const document = readReviewJson(filePath)
    const markdown = exportMarkdown(document, exportOptionsFromArgs(args))
    fs.mkdirSync(path.dirname(path.resolve(out)), { recursive: true })
    fs.writeFileSync(out, markdown, 'utf8')
    console.log(`Wrote ${out}`)
  }
  console.log(`Exported ${files.length} Markdown file(s).`)
}

function exportOptionsFromArgs (args) {
  return {
...
function listPdfFiles (folder, recursive) {
  return listFiles(folder, recursive, filePath => /\.pdf$/i.test(filePath))
}

function listReviewJsonFiles (folder, recursive) {
  return listFiles(folder, recursive, filePath => /\.review\.json$/i.test(filePath) || (/\.json$/i.test(filePath) && !/package(-lock)?\.json$/i.test(path.basename(filePath))))
}

function listFiles (folder, recursive, predicate) {
  const entries = fs.readdirSync(folder, { withFileTypes: true })
  const results = []
  for (const entry of entries) {
    const full = path.join(folder, entry.name)
    if (entry.isDirectory() && recursive) results.push(...listFiles(full, recursive, predicate))
    if (entry.isFile() && predicate(full)) results.push(full)
  }
  return results.sort()
}

function ensureAtLeastOneFile (files, folder, kind) {
  if (files.length === 0) throw new Error(`No ${kind} files found in ${folder}`)
}

function reviewJsonRelativeMarkdownPath (inputFolder, filePath) {
  const rel = path.relative(inputFolder, filePath)
  if (/\.review\.json$/i.test(rel)) return rel.replace(/\.review\.json$/i, '.md')
  return rel.replace(/\.json$/i, '.md')
}
...
async function main () {
  ...
  const commands = {
    ...
    'review-folder': commandReviewFolder,
    'export-folder': commandExportFolder
  }
