...
For users who prefer a window instead of typing paths, this repo includes a small Tkinter GUI wrapper. It opens file dialogs for input and output, then calls the same CLI internally.
...
- Convert one PDF → Markdown
- Create one review JSON from one PDF
- Export one review JSON → Markdown
- Convert a whole PDF folder → Markdown folder
- Create review JSON files for a whole PDF folder
- Export a whole review JSON folder → Markdown folder
- Folder option: include subfolders recursively
- Export options: body only, safe area only, translated text
...
# Batch review JSON generation
unified-pdf2md review-folder --inputFolderPath ./pdfs --outputFolderPath ./review-json --recursive

# Batch export from review JSON to Markdown
unified-pdf2md export-folder --inputFolderPath ./review-json --outputFolderPath ./md --recursive --body-only
```
