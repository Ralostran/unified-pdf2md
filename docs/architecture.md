...
`apps/gui-python` is a thin Tkinter desktop wrapper. It is intentionally not a separate conversion engine. It only collects paths/options from file dialogs and invokes `apps/cli/bin/unified-pdf2md.js`.

Architectural reason: this preserves one production conversion path, keeps batch CLI and GUI behavior consistent, and avoids introducing a heavy Electron dependency before the interactive reviewer UI is implemented.
